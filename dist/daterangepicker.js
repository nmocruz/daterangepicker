...
(function(root, factory) {
    ...
    ko.bindingHandlers.daterangepicker = (function() {
        return $.extend(this, {
            ...
            update: function(element, valueAccessor, allBindings) {
                var $element, allEvents, dateFormat, endDate, endDateText, ref1, startDate, startDateText;
                $element = $(element);
                ref1 = valueAccessor()(), startDate = ref1[0], endDate = ref1[1], allEvents = ref1[2];
    ...
    Period = (function() {
        ...
        Period.title = function(period, localeObj) {
            switch (period) {
                case 'day':
    ...
    Config = (function() {
        ...
        Config.prototype._allEvents = function(val) {
        ...
        Config.prototype.defineRange = function(range) {
            var result;

            if (!(range instanceof $.fn.daterangepicker.DateRange)) {
                throw new Error('Should be a DateRange instance');
            }

            switch (range.title) {
                case 'All Time':
                    return new DateRange(range.title, this.minDate().clone(), this.maxDate().clone());
                    break;
                case 'Custom Range':
                    return new CustomDateRange(range.title);
                    break;
                default:
        ...
        Config.prototype.parseRange = function(range) {
            var endDate, from, startDate, to;
            var DateRange = $.fn.daterangepicker.DateRange;

            if (!(range instanceof DateRange)) {
                throw new Error('Should be an instance of DateRange');
            }
            ...
            return new DateRange(range.title, from, to);
        };
    ...
    CalendarHeaderView = (function() {
        function CalendarHeaderView(calendarView) {
            ...
            this.prevDate = ko.pureComputed((function(_this) {
                return function() {
                    var amount, period, ref1;
                    ref1 = _this.period.nextPageArguments(), amount = ref1[0], period = ref1[1];
        ...
        CalendarHeaderView.prototype.nextArrowCss = function() {
            var cols, date, ref1, ref2, rows;
            ref1 = this.period.dimentions(), cols = ref1[0], rows = ref1[1];
            date = this.firstDate().clone().add(cols * rows, this.period());
            if ((ref2 = this.period()) === 'day' || ref2 === 'week') {
        ...
        CalendarHeaderView.prototype.monthOptions = function() {
            var l, maxMonth, minMonth, results;
            ...
            return (function() {
                results = [];
                for (var l = minMonth; minMonth <= maxMonth ? l <= maxMonth : l >= maxMonth; minMonth <= maxMonth ? l++ : l--) {
                    results.push(l);
                }
                return results;
            }).apply(this);
        ...
        CalendarHeaderView.prototype.yearOptions = function() {
            ...
            return (function() {
                results = [];
                for (var l = ref1 = this.currentDate.minBoundary().year(), ref2 = this.currentDate.maxBoundary().year(); ref1 <= ref2 ? l <= ref2 : l >= ref2; ref1 <= ref2 ? l++ : l--) {
                    results.push(l);
                }
                return results;
            }).apply(this);
        ...
        CalendarHeaderView.prototype.decadeFormatter = function(from) {
            var cols, ref1, rows, to;
            ref1 = Period.dimentions('year'), cols = ref1[0], rows = ref1[1];
            to = from + cols * rows - 1;
            return from + " – " + to;
        };
    ...
    CalendarView = (function() {
        function CalendarView(mainView, dateSubscribable, type) {
            ...
            this.isEvent = bind(this.isEvent, this);
        ...
        CalendarView.prototype.calendar = function() {
            var col, cols, date, iterator, l, ref1, ref2, results, row, rows;
            ref1 = this.period.dimentions(), cols = ref1[0], rows = ref1[1];
            ...
            for (row = l = 1, ref2 = rows; 1 <= ref2 ? l <= ref2 : l >= ref2; row = 1 <= ref2 ? ++l : --l) {
                results.push((function() {
                    var m, ref3, results1;
                    results1 = [];
                    for (col = m = 1, ref3 = cols; 1 <= ref3 ? m <= ref3 : m >= ref3; col = 1 <= ref3 ? ++m : --m) {
                        date = iterator.next();
                        if (this.type === 'end') {
        ...
        CalendarView.prototype.cssForDate = function(date, periodIsDay) {
            var differentMonth, inRange, isEvent, obj1, onRangeEnd, withinBoundaries;
            ...
            //isEvent = this.isEvent(date);
            return (
                obj1 = {
                    "in-range": !this.single() && (inRange || onRangeEnd)
                },
                obj1[this.type + "-date"] = onRangeEnd,
                obj1["highlight"] = !this.single() && (inRange || onRangeEnd),
    ...
    DateRangePickerView = (function() {
        function DateRangePickerView(options) {
            var endDate, ref1, startDate, wrapper;
            if (options == null) {
                options = {};
            }
        ...
        DateRangePickerView.prototype.getLocale = function() {
            return this.locale;
        };
        ...
        <div class="daterangepicker" data-bind="css: $data.cssClasses(), style: $data.style()">
            <div class="controls">
                <ul class="periods">
                    ...
                    <li
                        class="period"
                        data-bind="css: {active: $parent.isActivePeriod($data) && !$parent.isCustomPeriodRangeActive()},
                            text: $parent.periodProxy.title($data, $parent.getLocale()),
                            click: function(){ $parent.setPeriod($data); }">
                    </li>
                    <!-- /ko -->
                    <!-- ko foreach: $data.customPeriodRanges -->
                    <li
                        class="period"
                        data-bind="css: {active: $parent.isActiveCustomPeriodRange($data)},
                            text: $data.title,
                            click: function(){ $parent.setCustomPeriodRange($data); }">
                    </li>
                    <!-- /ko -->
                </ul>
                <ul class="ranges" data-bind="foreach: $data.ranges">
                    <li
                        class="range"
                        data-bind="css: {active: $parent.isActiveDateRange($data)},
                            text: $data.title,
                            click: function(){ $parent.setDateRange($data); }">
                    </li>
                </ul>
            ...
            <div class="calendar">
                ...
                <div class="calendar-header" data-bind="with: $data.headerView">
                    <div class="arrow" data-bind="css: $data.prevArrowCss()">
                        <button data-bind="click: $data.clickPrevButton">
                    ...
                    <div class="calendar-selects">
                        <select
                            class="month-select"
                            data-bind="options: $data.monthOptions(),
                                optionsText: $data.monthFormatter,
                                valueAllowUnset: true,
                                value: $data.selectedMonth,
                                fireChange: true,
                                css: {hidden: !$data.monthSelectorAvailable()}">
                        </select>
                        <select
                            class="year-select"
                            data-bind="options: $data.yearOptions(),
                                optionsText: $data.yearFormatter,
                                valueAllowUnset: true,
                                value: $data.selectedYear,
                                fireChange: true,
                                css: {hidden: !$data.yearSelectorAvailable()}">
                        </select>
                        <select
                            class="decade-select"
                            data-bind="options: $data.decadeOptions(),
                                optionsText: $data.decadeFormatter,
                                valueAllowUnset: true,
                                value: $data.selectedDecade,
                                fireChange: true,
                                css: {hidden: !$data.decadeSelectorAvailable()}">
                        </select>
                    </div>
                    <div class="arrow" data-bind="css: $data.nextArrowCss()">
                        <button data-bind="click: $data.clickNextButton">
